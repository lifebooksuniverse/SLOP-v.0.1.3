...
# Rich console is way easier to use than tkinter. that was a pain to do previously.
console = Console()
...
ERROR_FOLDER_NAME = "error" #self explanatory
...
def load_image(img_path: Path) -> Optional[Image.Image]:
    # WHAT: Wraps the image loading process in a try/except block.
    # WHY: Prevents a single corrupted or truncated file from crashing the entire batch sorting process.
    try:
        ...
        if ext in RAW_EXTENSIONS:
            ...
            with rawpy.imread(str(img_path)) as raw:
                # WHAT: Extracts a downsampled RGB array directly from the RAW file.
                # WHY: Full demosaicing takes seconds per image; half_size keeps the model input fast while preserving useful detail.
                try:
        ...
        # WHAT: Opens standard images with a robust load step.
        # WHY: Some files are truncated or partially corrupted and fail only when pixel data is actually accessed.
...
def select_directory(start_dir: Path, title: str = "Select destination folder") -> Optional[Path]:
    ...
    if not base_dir.exists():
        base_dir = home_dir
    if not is_safe_under_home(base_dir):
        base_dir = home_dir
    ...
    while True:
        ...
        if not is_safe_under_home(current_dir):
            current_dir = home_dir
    ...
    try:
        ...
        if not output_dir:
            ...
            if not is_safe_under_home(output_path):
                output_dir = str(source_path / "sorted")
            elif str(output_path) == str(Path("./sorted_photos").expanduser()):
                output_dir = str(source_path / "sorted")
...
# [fast_resort_from_log function remains unchanged...]
...
class photosorter:
    ...
    def classify_batch(self, img_paths: List[Path], categories: List[str]) -> List[Tuple[Path, str, float, str]]:
        # WHAT: Compiles multiple images into a single inference payload.
        # WHY: Pushing data to the GPU in chunks (batches) is exponentially faster than sending one image at a time.
        images = []
        valid_paths = []
        ...
        for p in img_paths:
            ...
            if img is not None:
                ...
                # WHAT: Tracks recoverable load failures without aborting the whole batch.
                # WHY: Corrupt or truncated images should be quarantined, not crash the sort job.
                pass
        ...
        for i in range(len(valid_paths)):
            ...
            results.append((valid_paths[i], categories[best_idx], confidence, "sorted"))
...
class SLOPTUI:
    ...
    def make_header(self) -> Panel:
        ...
        stats_text = (
            ...
            f"VRAM {vram_bar} [green]{vram_used_mb:.1f} MB[/green]/[yellow]{vram_total_mb:.1f} MB[/yellow] ({vram_pct:.0f}%) | "
...
def edit_config_menu(categories: List[str], use_gpu: bool, threads: int, batch_size: int, source_dir: Path, output_dir: Path, log_path: Path, config_path: Path) -> Tuple[List[str], bool, int, int, Path, Path, Path]:
    ...
    while True:
        ...
        choice = Prompt.ask("\nSelect action", choices=valid_choices, default="10")
...
def main():
    ...
    while True:
            ...
            try:
                with Live(tui.build_layout(progress), refresh_per_second=8) as live:
                    ...
                    if session_images_processed > 0:
                        ...
                        summary_lines = summarize_sort_results(category_move_counts, elapsed_seconds, 0.0, broken_files)
                        console.print(Panel(
                            f"[bold cyan]Sorting complete[/bold cyan]\n"
                            f"[green]Processed:[/green] 0 image(s)\n"
                            f"[green]Sorted successfully:[/green] 0 image(s)\n"
                            f"[green]Errors:[/green] {broken_files} image(s)\n\n"
                            f"{summary_lines}",
                            border_style="cyan"
                        ))
        ...
        if not unprocessed_files:
            console.print("[bold green]All images in folder have already been sorted...[/bold green]")
            Prompt.ask("Press Enter to return to the main menu")
            continue
